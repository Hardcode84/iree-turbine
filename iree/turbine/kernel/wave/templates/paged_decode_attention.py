...
def get_paged_decode_attention_mha_kernels(
    shape: paged_decode_attention_shape,
    mfma_variant: tuple[MMAType, MMAType],
    num_kv_splits: int,
    k_shape: tuple[int],
    ...
    k_mapping = tkw.IndexMapping(
        num_iterators=4,
        inputs={S: d0 // K2_dim, B: j, K2: d0 % K2_dim, K1: l},
        outputs={S: i, B: j, K2: k, K1: l},
        dynamic_val_mappings={K2: k},
    )
    ...
    v_mapping = tkw.IndexMapping(
        num_iterators=4,
        inputs={S: d0 // K2_dim, B: j, N: k, K2: d0 % K2_dim},
        outputs={S: i, B: j, N: k, K2: l},
        dynamic_val_mappings={K2: l},
    )
    ...
    def phase_0(
        ...
        # =========================================================================
        # Query has shape [NUM_SEQS, NUM_HEADS, HEAD_DIM]
        # Key has shape [NUM_BLOCKS, NUM_HEADS, BLOCK_SIZE, HEAD_DIM]
        # Value has shape [NUM_BLOCKS, NUM_HEADS, HEAD_DIM, BLOCK_SIZE]
        # Sequence lengths has shape [NUM_SEQS]
        # Request indices has shape [NUM_SEQS]
        # Block table has shape [NUM_SEQS, MAX_KV_SEQ_LEN]
        # Output has shape [NUM_KV_SPLITS, NUM_SEQS, NUM_HEADS, HEAD_DIM]
